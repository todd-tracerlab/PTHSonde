...
\begin{tikzpicture}[
  ...
  \node[box,right=of sonde] (rx) {\textbf{Ground}\\\textbf{Receiver}\\[1pt]\footnotesize LoRa radio};
...
\begin{itemize}
  ...
  \item \textbf{Reusable payload} --- a rugged enclosure with a swappable battery; recover it and fly again.
...
The \PTH{} payload (shown on the cover) is a compact, reusable radiosonde carrying the
full sensor suite and the radio, with a removable battery door and an exposed thermistor
probe for true air temperature.
...
\rowcolor{tllight}
Pressure altitude (derived) & $0$--$31$\,km & $\pm10$\,m & $1$\,m & $<1$\,s \\
Wind speed (GPS-derived) & $0$--$100$\,m/s & $\pm0.5$\,m/s & $0.1$\,m/s & $3$\,s (downlink) \\
\rowcolor{tllight}
...
\begin{tip}
On power-up the status LED blinks while the radio and GPS initialise. On the dashboard,
confirm the \textbf{Sonde ID} appears and every sensor dot reads green before you close it up.
\end{tip}
...
The receiver is a small box with the same LoRa radio as the sonde, an SMA antenna
connector, and a USB lead to your laptop. Like the sonde, each receiver carries its
own hex ID on a printed sticker.

\begin{figure}[htbp]
\centering
\rotatebox{180}{\includegraphics[height=0.58\textheight]{gcs.png}}
...
\clearpage
\subsection{Field quick-start card}
The card below condenses the whole workflow --- payload, fill figures for the two
standard balloons, the launch sequence and troubleshooting --- onto one page.
\textbf{Print it at 100\% scale} (it is sized $3.62\times6.6$\,in) and cut it out to
carry in the field.

\begin{figure}[H]
\centering
\includegraphics[height=0.82\textheight]{quickstart_card.png}
\caption{Print-and-cut \PTH{} field quick-start card ($3.62\times6.6$\,in).}
\end{figure}
